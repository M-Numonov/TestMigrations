...
const Global_settingsTablesPage = () => {
  ...
  const [filters] = React.useState([
    { label: 'Name', title: 'name' },
    {
      ...
      title: 'subscription_expiry_notification_days',
      number: 'true',
    },
    {
      label: 'Billing Cycle Grace Period',
      title: 'billing_cycle_grace_period',
      number: 'true',
    },
  ]);
  ...
  const getGlobal_settingsCSV = async () => {
    const response = await axios({
      url: '/global_settings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox className='mb-6'>
          <BaseButton
            className={'mr-3'}
            href={'/global_settings/global_settings-new'}
            color='info'
            label='New Item'
          />
          <BaseButton
