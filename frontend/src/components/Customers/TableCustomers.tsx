...
import { fetch, deleteItem } from '../../stores/customers/customersSlice'
...
const TableSampleCustomers = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { customers, loading, count } = useAppSelector((state) => state.customers)
  ...
  const customersPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
      if (id) {
          ...
          setIsModalTrashActive(false);
          notify('success', 'Customers was deleted!')
      }
  };
    ...
    {filterItems && filterItems.length ?
        <CardBox>
            ...
            >
                <Form>
                    <>
                        {customers && customers.map((filterItem) => {
                            return (
                                <>
        ...
        <tbody>
          {customers && Array.isArray( customers ) &&
            customers.map((item: any) => (
              <tr key={item.id} onClick={() => router.push(`/customers/${item.id}`)}>

                <td data-label="name">
                  { item.name }
                </td>

                <td data-label="current_period_starts">
                  { dataFormatter.dateTimeFormatter(item.current_period_starts) }
                </td>
