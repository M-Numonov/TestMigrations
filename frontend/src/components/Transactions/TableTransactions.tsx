...
import { fetch, deleteItem } from '../../stores/transactions/transactionsSlice'
...
const TableSampleTransactions = ({ filterItems, setFilterItems, filters }) => {
  ...
  const transactionsPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
      if (id) {
          ...
          setIsModalTrashActive(false);
          notify('success', 'Transactions was deleted!')
      }
  };
    ...
    {filterItems && filterItems.length ?
        <CardBox>
            ...
            >
                <Form>
                    <>
                        {transactions && transactions.map((filterItem) => {
                            return (
                                <>
        ...
        <tbody>
          {transactions && Array.isArray( transactions ) &&
            transactions.map((item: any) => (
              <tr key={item.id} onClick={() => router.push(`/transactions/${item.id}`)}>
      ...
      <div className="p-3 lg:px-6 border-t border-gray-100 dark:border-slate-800">
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <BaseButtons>
            {transactionsPaginated.map((page) => (
              <BaseButton
                key={page}
