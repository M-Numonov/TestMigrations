...
import { fetch, deleteItem } from '../../stores/subscription_plans/subscription_plansSlice'
...
const TableSampleSubscription_plans = ({ filterItems, setFilterItems, filters }) => {
  ...
  const { subscription_plans, loading, count } = useAppSelector((state) => state.subscription_plans)

  const numPages = Math.floor(count / perPage) === 0 ? 1 : Math.floor(count / perPage);
  ...
  const subscription_plansPaginated = Array.from(
  ...
  const handleDeleteAction = async () => {
      if (id) {
          ...
          setIsModalTrashActive(false);
          notify('success', 'Subscription_plans was deleted!')
      }
  };
    ...
    const handleSubmit = (e) => {
        ...
        filterItems.forEach((item) => {
            filters[
                filters.map((filter) => filter.title).indexOf(item.fields.selectedField)].number
    ...
    {filterItems && filterItems.length ?
        <CardBox>
            ...
            >
                <Form>
                    <>
                        {subscription_plans && subscription_plans.map((filterItem) => {
                            return (
                                <>
      ...
      <table>
        ...
        <tbody>
          {subscription_plans && Array.isArray( subscription_plans ) &&
            subscription_plans.map((item: any) => (
              <tr key={item.id} onClick={() => router.push(`/subscription_plans/${item.id}`)}>

                <td data-label="name">
                  { item.name }
                </td>

                <td data-label="trial_period">
                  { item.trial_period }
                </td>

                <td data-label="billing_cycle">
                  { item.billing_cycle }
                </td>
      ...
      <div className="p-3 lg:px-6 border-t border-gray-100 dark:border-slate-800">
        <div className="flex flex-col md:flex-row items-center justify-between py-3 md:py-0">
          <BaseButtons>
            {subscription_plansPaginated.map((page) => (
              <BaseButton
                key={page}
...
export default TableSampleSubscription_plans
