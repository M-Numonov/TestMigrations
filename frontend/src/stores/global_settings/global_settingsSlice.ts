...
export const fetch = createAsyncThunk(
  'global_settings/fetch',
  async (data: any) => {
    const { id, query } = data;
    const result = await axios.get(
      `global_settings${query || (id ? `/${id}` : '')}`,
    );
    return id
      ? result.data
...
export const deleteItem = createAsyncThunk(
  'global_settings/deleteGlobal_settings',
  async (id: string) => {
    try {
...
export const create = createAsyncThunk(
  'global_settings/createGlobal_settings',
  async (data: any) => {
    const result = await axios.post('global_settings', { data });
...
export const update = createAsyncThunk(
  'global_settings/updateGlobal_settings',
  async (payload: any) => {
    const result = await axios.put(`global_settings/${payload.id}`, {
      id: payload.id,
      data: payload.data,
    });
    return result.data;
